...
export default function RegisterForm({
  ...
  return (
    <form
      ...
      className="grid grid-cols-2 justify-center gap-3"
    >
      {registerFormFields.map((field, index) => (
        <div
          key={index}
          className={`flex flex-col w-full md:size-auto ${
            field.id === 3 ? "col-span-2" : ""
          } `}
        >
          ...
          <input
            ...
            className={`w-full md:size-auto border ${
              registerErrors[field.name] ? "border-red-500" : "border-gray-800"
            } py-2 pr-3 pl-2 focus:outline-none`}
          />
          <span className="text-sm pt-2">
            {registerErrors[field.name] && registerErrors[field.name][0]}
          </span>
        </div>
      ))}
      <div className="pt-3">
